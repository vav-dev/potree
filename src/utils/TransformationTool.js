...
export class TransformationTool {
  ...
  initializeFocusHandles() {
    ...
    let texture = new THREE.TextureLoader().load(
      `${exports.resourcePath}/icons/eye_2.png`
    );
  ...
  setActiveHandle(handle) {
    ...
    for (let handleName of Object.keys(this.scaleHandles)) {
      ...
      if (this.activeHandle === handle) {
        ...
        let relatedFocusHandle =
          this.focusHandles[handle.name.replace("scale", "focus")];
        ...
        for (let translationHandleName of Object.keys(
          ...
          let translationHandle =
            this.translationHandles[translationHandleName];
