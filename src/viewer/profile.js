    ...
    for (let i = 0; i < data.numPoints; i++) {
      ...
      for (let attributeName of Object.keys(data.data)) {
        ...
        for (let item = 0; item < numElements; item++) {
          target.array[numElements * index + item] =
            source[numElements * i + item];
        }
      }
    ...
    for (let attributeName of Object.keys(data.data)) {
      ...
      if (this.trueOctree.root.sceneNode) {
        if (this.trueOctree.root.sceneNode.geometry.attributes[attributeName]) {
          normalized =
            this.trueOctree.root.sceneNode.geometry.attributes[attributeName]
              .normalized;
        }
      }
...
export class ProfileWindow extends EventDispatcher {
  ...
  initListeners() {
        ...
        if (closest) {
          ...
          for (let attributeName of Object.keys(point)) {
            ...
            if (attributeName === "position") {
              let values = [...position].map((v) =>
                Utils.addCommas(v.toFixed(3))
              );
              html += `
								<tr>
									<td>x</td>
    ...
    $("#potree_download_dxf2D_icon").click(() => {
      ...
      $("#potree_download_profile_dxf2D_link").attr(
        "href",
    ...
    $("#potree_download_dxf3D_icon").click(() => {
      ...
      const blob = new Blob([string], { type: "text/string" });
      $("#potree_download_profile_dxf3D_link").attr(
        "href",
        URL.createObjectURL(blob)
      );
    });
    ...
    for (let [pointcloud, entry] of this.pointclouds) {
      for (let points of entry.points) {
        ...
        for (let i = 0; i < points.numPoints; i++) {
          ...
          let e =
            points.data.position[3 * i + 2] - elevation + pointcloud.position.z;
          ...
          if (points.data.classification) {
            ...
            if (
              pointClassValue &&
              (!pointClassValue.visible || pointClassValue.color.w === 0)
            ) {
...
export class ProfileWindowController {
  constructor(viewer) {
    ...
    const rotate = (radians) => {
      ...
      const mMoveOrigin = new THREE.Matrix4().makeTranslation(
        -center.x,
        -center.y,
        -center.z
      );
      const mRotate = new THREE.Matrix4().makeRotationZ(radians);
      const mMoveBack = new THREE.Matrix4().makeTranslation(
        center.x,
        center.y,
        center.z
      );
