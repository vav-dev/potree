  ...
  async download() {
    ...
    let regions = [];
    {
      ...
      for (let segment of segments) {
        ...
        let startEndDir = new THREE.Vector3()
          .subVectors(end, start)
          .normalize();
        let endStartDir = new THREE.Vector3()
          .subVectors(start, end)
          .normalize();
    ...
    for (let pointcloud of this.viewer.scene.pointclouds) {
      ...
      let negateOffset = new THREE.Matrix4().makeTranslation(
        ...offset.multiplyScalar(-1).toArray()
      );
      let matrixWorld = pointcloud.matrixWorld;

      let transform = new THREE.Matrix4().multiplyMatrices(
        matrixWorld,
        negateOffset
      );
