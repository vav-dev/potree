...
export class CameraAnimation extends EventDispatcher {
      ...
      for (const cp of this.controlPoints) {
        {
          ...
          if (visible) {
            ...
            cp.positionHandle.svg.style.left =
              x - cp.positionHandle.svg.clientWidth / 2;
            cp.positionHandle.svg.style.top =
...

        {
          ...
          if (visible) {
            ...
            cp.targetHandle.svg.style.left =
              x - cp.targetHandle.svg.clientWidth / 2;
            cp.targetHandle.svg.style.top =
  ...
  createControlPoint(index) {
    ...
    if (this.controlPoints.length >= 2 && index === 0) {
      ...
      cp.position.copy(
        cp1.position.clone().add(cp2.position).multiplyScalar(0.5)
      );
  ...
  createHandle(vector) {
    ...
    circle.setAttributeNS(
      null,
      "style",
      "fill: red; stroke: black; stroke-width: 0.2em;"
    );
    ...
    const drag = (evt) => {
      if (this.selectedElement) {
        ...
        const { width, height } = this.viewer.renderer.getSize(
          new THREE.Vector2()
        );
