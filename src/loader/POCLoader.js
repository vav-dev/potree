...
function lasLazAttributes(fMno) {
  ...
  attributes.add(
    new PointAttribute("rgba", PointAttributeTypes.DATA_TYPE_UINT8, 4)
  );
  attributes.add(
    new PointAttribute("intensity", PointAttributeTypes.DATA_TYPE_UINT16, 1)
  );
  attributes.add(
    new PointAttribute("classification", PointAttributeTypes.DATA_TYPE_UINT8, 1)
  );
  attributes.add(
    new PointAttribute("gps-time", PointAttributeTypes.DATA_TYPE_DOUBLE, 1)
  );
  attributes.add(
    new PointAttribute(
      "number of returns",
  ...
  attributes.add(
    new PointAttribute("return number", PointAttributeTypes.DATA_TYPE_UINT8, 1)
  );
  attributes.add(
    new PointAttribute("source id", PointAttributeTypes.DATA_TYPE_UINT16, 1)
  );
        ...
        if (xhr.readyState === 4 && (xhr.status === 200 || xhr.status === 0)) {
          ...
          if (version.upTo("1.4")) {
            for (let i = 1; i < fMno.hierarchy.length; i++) {
              ...
              let boundingBox = Utils.createChildAABB(
                parentNode.boundingBox,
                index
              );

              let node = new PointCloudOctreeGeometryNode(
                name,
                pco,
                boundingBox
              );
