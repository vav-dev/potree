...
export class GeoPackageLoader {
  ...
  static async loadUrl(url, params) {
    await Promise.all([
      Utils.loadScript(
        `${Potree.scriptPath}/lazylibs/geopackage/geopackage.js`
      ),
  ...
  static async loadBuffer(buffer, params) {
    ...
    const resolver = async (resolve) => {
      ...
      for (const table of tables.features) {
        ...
        boundingBox = boundingBox.projectBoundingBox(
          dao.projection,
          "EPSG:4326"
        );
        ...
        for (const [index, feature] of Object.entries(geoJson)) {
          ...
          const featureNode = GeoPackageLoader.featureToSceneNode(
            feature,
            matLine,
            dao.projection,
            transform
    ...
    if (feature.geometry.type === "Point") {
      ...
      for (let pc of geometry.coordinates) {
        ...
        for (let i = 0; i < pc.length; i++) {
          ...
          let pos = transform.forward(
            geopackageProjection.forward([long, lat])
          );
