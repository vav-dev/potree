...
export class Annotation extends EventDispatcher {
  ...
  installHandles(viewer) {
    ...
    $(this.domElement).draggable({
      ...
      drag: (event, ui) => {
        let renderAreaWidth = viewer.renderer.getSize(
          new THREE.Vector2()
        ).width;
    ...
    let updateCallback = () => {
      ...
      let toScreen = (position) => {
        ...
        let worldView = new THREE.Matrix4().multiplyMatrices(
          camera.projectionMatrix,
          camera.matrixWorldInverse
        );
        let ndc = new THREE.Vector4(
          position.x,
          position.y,
          position.z,
          1.0
        ).applyMatrix4(worldView);
