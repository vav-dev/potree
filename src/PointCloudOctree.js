...
export class PointCloudOctree extends PointCloudTree {
  constructor(geometry, material) {
...

    {
      ...
      for (let attributeName of priorityQueue) {
        ...
        let min =
          attribute.range[0].constructor.name === "Array"
            ? attribute.range[0]
            : [attribute.range[0]];
        let max =
          attribute.range[1].constructor.name === "Array"
            ? attribute.range[1]
            : [attribute.range[1]];
  ...
  deepestNodeAt(position) {
    ...
    while (true) {
      ...
      if (
        containingChild !== null &&
        containingChild instanceof PointCloudOctreeNode
      ) {
  ...
  intersectsPoint(position) {
    ...
    if (typeof this.signedDistanceField === "undefined") {
      ...
      for (let i = 0; i < n; i = i + 3) {
        ...
        const ix = parseInt(
          Math.min(resolution * (x / boundingBox.max.x), resolution - 1)
        );
        const iy = parseInt(
          Math.min(resolution * (y / boundingBox.max.y), resolution - 1)
        );
        const iz = parseInt(
          Math.min(resolution * (z / boundingBox.max.z), resolution - 1)
        );
  ...
  pick(viewer, camera, ray, params = {}) {
...

    {
      ...
      pickMaterial.uniforms.minSize.value =
        this.material.uniforms.minSize.value;
      pickMaterial.uniforms.maxSize.value =
        this.material.uniforms.maxSize.value;
