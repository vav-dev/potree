  ...
  renderNodes(
    ...
    for (let node of nodes) {
      ...
      if (isExtraAttribute) {
        ...
        {
          const attExtra = octree.pcoGeometry.pointAttributes.attributes.find(
            (a) => a.name === attName
          );
        ...
        for (const attributeName in geometry.attributes) {
          ...
          if (attributeLocations[attributeName] !== undefined) {
            ...
            let type = this.glTypeMapping.get(
              bufferAttribute.array.constructor
            );
            let normalized = bufferAttribute.normalized;
      ...
      //if(material.needsUpdate){
      {
        ...
        let numClipBoxes =
          material.clipBoxes && material.clipBoxes.length
            ...
            : 0;
        let numClipSpheres =
          params.clipSpheres && params.clipSpheres.length
            ? params.clipSpheres.length
            : 0;
        let numClipPolygons =
...

    {
      ...
      if (params.clipSpheres && params.clipSpheres.length > 0) {
        ...
        for (let clipSphere of clipSpheres) {
          ...
          let viewToClip = new THREE.Matrix4().multiplyMatrices(
            worldToClip,
            viewToWorld
          );
        ...
        let flattenedMatrices = [].concat(
          ...matrices.map((matrix) => matrix.elements)
        );
      ...
      if (material.snapEnabled === true) {
        {
          ...
          for (let i = 0; i < 5; i++) {
            ...
            let snapTexture =
              this.threeRenderer.properties.get(texture).__webglTexture;
            let snapTextureDepth =
              this.threeRenderer.properties.get(textureDepth).__webglTexture;
