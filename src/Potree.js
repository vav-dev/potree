...
Potree.pointBudget = 1 * 1500 * 1000;
...
const needRenderFuncs = [];

export const needRender = () => {
  needRenderFuncs.forEach((x) => x());
};

export function addRenderFunc(fn) {
  needRenderFuncs.push(fn);
}

export function setPointsBudget(pointsBudget) {
  Potree.pointBudget = pointsBudget;
}

export function getPointsBudget() {
