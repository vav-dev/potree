...
vec3 getElevation(){
	...
	float w = (world.y - elevationRange.x) / (elevationRange.y - elevationRange.x);
...
void doClipping(){
	...
	int insideCount = 0;
		...
		for(int i = 0; i < num_clipboxes; i++){
			vec4 clipPosition = inverse(clipBoxes[i]) * modelMatrix * vec4( position, 1.0 );
